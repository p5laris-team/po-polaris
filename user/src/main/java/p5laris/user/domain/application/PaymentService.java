...
public class PaymentService {
     ...
     */
    @Transactional
    public int refundPayment(Long userId, String orderNo, String reason) {
        PaymentOrder order = orderRepository.findByOrderNo(orderNo)
                .orElseThrow(() -> new UserException(UserErrorCode.PAYMENT_ORDER_NOT_FOUND));
        ...
        PaymentTransaction transaction = transactionRepository.findByPaymentOrderId(order.getId())
                .orElseThrow(() -> new UserException(UserErrorCode.PAYMENT_TRANSACTION_FAILED));
        ...
        if (!mockMode) {
            try {
                ...
                HttpRequest request = HttpRequest.newBuilder()
                        .uri(URI.create("https://api.portone.io/payments/" + transaction.getPaymentId() + "/cancel"))
                        .header("Authorization", "PortOne " + apiSecret)
                        .header("Content-Type", "application/json")
                ...
                HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
                if (response.statusCode() != 200) {
                    log.error("Portone V2 Cancel response error. Status: {}, Body: {}", response.statusCode(), response.body());
                    throw new UserException(UserErrorCode.PAYMENT_TRANSACTION_FAILED);
                }
                ...
                log.error("Portone V2 refund failed due to exception", e);
                throw new UserException(UserErrorCode.PAYMENT_TRANSACTION_FAILED);
            }
        } else {
            log.info("[MOCK MODE] Payment refund/cancel bypassed for orderNo={}", orderNo);
        }
        ...
        transaction.cancel(LocalDateTime.now(), order.getAmount());
        transactionRepository.save(transaction);

        // 별조각 회수
        walletService.spendStarPiece(
                userId,
                order.getStarPieces(),
                "PAYMENT_REFUND",
                "PAYMENT_ORDER",
