-- =============================================================
-- Polaris Character Domain - Initial Schema (Unified V1)
-- =============================================================

-- 1. character_types (MVP 캐릭터 3종 NOVA, MUMU, JJORY)
...
-- 2. character_assets (캐릭터 타입별 이미지 에셋)
...
-- 3. user_characters (사용자가 키우는 캐릭터 인스턴스, V3 반영)
CREATE TABLE user_characters (
    ...
    last_stat_decreased_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    created_at          TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at          TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE UNIQUE INDEX uq_user_characters_user_active ON user_characters (user_id) WHERE active = TRUE;

-- 4. character_care_logs (돌봄 액션 기록, V5 반영)
...
CREATE UNIQUE INDEX uq_character_care_logs_idempotency_key ON character_care_logs (idempotency_key);

-- 5. share_cards (공유 카드 정보, V7 반영)
...
CREATE UNIQUE INDEX uq_share_cards_user_image_url ON share_cards (user_id, image_url) WHERE image_url IS NOT NULL;

-- 6. share_logs (SNS 공유 이벤트 + 보상 로그, V6 반영)
...
CREATE UNIQUE INDEX uq_share_logs_daily_reward_paid ON share_logs (user_id, share_date) WHERE reward_paid = TRUE;

-- 7. character_outbox_events (아웃박스 이벤트, V13 반영)
CREATE TABLE character_outbox_events (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uk_character_outbox_events_idempotency_key UNIQUE (idempotency_key),
    CONSTRAINT chk_character_outbox_events_status CHECK (
        status IN ('PENDING', 'PROCESSING', 'SUCCEEDED', 'FAILED')
    )
);

CREATE INDEX idx_character_outbox_events_status_next_attempt ON character_outbox_events(status, next_attempt_at);
CREATE INDEX idx_character_outbox_events_event_status_next_attempt ON character_outbox_events(event_type, status, next_attempt_at);
CREATE INDEX idx_character_outbox_events_aggregate ON character_outbox_events(aggregate_type, aggregate_id, event_type);

-- 8. character_exp_logs (경험치 로그, V14 반영)
CREATE TABLE character_exp_logs (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uk_character_exp_logs_idempotency_key UNIQUE (idempotency_key),
    CONSTRAINT uk_character_exp_logs_source UNIQUE (source_type, source_id)
);

CREATE INDEX idx_character_exp_logs_character_created_at ON character_exp_logs(character_id, created_at);
CREATE INDEX idx_character_exp_logs_user_created_at ON character_exp_logs(user_id, created_at);

-- 9. character_story_fragments (스토리 프래그먼트, V15 반영)
CREATE TABLE character_story_fragments (
    ...
    CONSTRAINT chk_character_story_fragments_character_type CHECK (character_type_code IN ('MUMU', 'NOVA', 'JJORY', 'COMMON')),
    CONSTRAINT chk_character_story_fragments_fragment_type CHECK (fragment_type IN ('COMMON', 'LORE', 'EASTER_EGG')),
    CONSTRAINT chk_character_story_fragments_trigger_type CHECK (trigger_type IN ('TAP', 'LEVEL_UP', 'LOW_HUNGER', 'LOW_ENERGY', 'LOW_AFFECTION', 'NIGHT', 'MIDNIGHT'))
);

CREATE INDEX idx_character_story_fragments_lookup ON character_story_fragments(character_type_code, trigger_type, min_level, active);

-- 10. user_character_story_unlocks (스토리 해금 내역, V15 반영)
CREATE TABLE user_character_story_unlocks (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uk_user_character_story_unlocks_fragment UNIQUE (user_id, user_character_id, story_fragment_id)
);

CREATE INDEX idx_user_character_story_unlocks_user_character ON user_character_story_unlocks(user_id, user_character_id, unlocked_at);
...
-- Seed MVP 캐릭터 3종 (V2)
INSERT INTO character_types (code, name, summary, personality, speech_style, intro_message, sample_line, active, sort_order) VALUES
('NOVA', '노바', '자기가 별이었다는 걸 까먹은 별알', '다정함, 조심스러움, 기억이 듬성듬성함', '짧고 느림. 문장 끝이 작게 흐려짐', '노바는 자기가 한때 길을 비추던 별이었다는 걸 까먹은 별알이에요. 말은 조금 느리지만, 당신이 작은 일을 해낼 때마다 조금씩 빛을 되찾아요.', '오늘도… 있었네.', TRUE, 1),
('MUMU', '무무', '“무…”밖에 못 하지만 다 알고 있는 작은 별나무', '말 없음, 기다림 많음, 은근 다정함', '거의 “무”만 말함. 시스템이 해석을 보조함', '무무는 말을 많이 하지 못해요. 하지만 당신이 작은 일을 해내면 잎이 조금 움직여요. 무무의 “무...”는 가끔 고맙다는 뜻일지도 몰라요.', '무…', TRUE, 2),
('JJORY', '쪼리', '현관까지 가면 세계여행이라고 믿는 별쥐', '시크한 척함, 겁 많음, 허세 있지만 귀여움', '건조함, 짧은 농담, X 감성', '쪼리는 늘 가방을 메고 있지만 멀리 가진 못해요. 그래도 현관까지 가는 일을 아주 큰 모험이라고 믿어요. 작은 행동을 너무 크게 부풀리는 재능이 있어요.', '집 앞도 밖임. 반박 안 받음.', TRUE, 3);

-- Seed 에셋 6종 (V8 CDN 경로 반영 및 동적 외래키 적용)
INSERT INTO character_assets (character_type_id, asset_type, asset_url) VALUES
((SELECT id FROM character_types WHERE code = 'NOVA'), 'IDLE', 'assets/characters/nova/core/character-nova-idle.png'),
((SELECT id FROM character_types WHERE code = 'NOVA'), 'HAPPY', 'assets/characters/nova/core/character-nova-happy.png'),
((SELECT id FROM character_types WHERE code = 'NOVA'), 'SLEEPY', 'assets/characters/nova/core/character-nova-sleepy.png'),
((SELECT id FROM character_types WHERE code = 'NOVA'), 'HUNGRY', 'assets/characters/nova/status/character-nova-hungry.png'),
((SELECT id FROM character_types WHERE code = 'NOVA'), 'LOW_ENERGY', 'assets/characters/nova/status/character-nova-low-energy.png'),
((SELECT id FROM character_types WHERE code = 'NOVA'), 'LONELY', 'assets/characters/nova/status/character-nova-lonely.png'),

((SELECT id FROM character_types WHERE code = 'MUMU'), 'IDLE', 'assets/characters/mumu/core/character-mumu-idle.png'),
((SELECT id FROM character_types WHERE code = 'MUMU'), 'HAPPY', 'assets/characters/mumu/core/character-mumu-happy.png'),
((SELECT id FROM character_types WHERE code = 'MUMU'), 'SLEEPY', 'assets/characters/mumu/core/character-mumu-sleepy.png'),
((SELECT id FROM character_types WHERE code = 'MUMU'), 'HUNGRY', 'assets/characters/mumu/status/character-mumu-hungry.png'),
((SELECT id FROM character_types WHERE code = 'MUMU'), 'LOW_ENERGY', 'assets/characters/mumu/status/character-mumu-low-energy.png'),
((SELECT id FROM character_types WHERE code = 'MUMU'), 'LONELY', 'assets/characters/mumu/status/character-mumu-lonely.png'),

((SELECT id FROM character_types WHERE code = 'JJORY'), 'IDLE', 'assets/characters/jjory/core/character-jjory-idle.png'),
((SELECT id FROM character_types WHERE code = 'JJORY'), 'HAPPY', 'assets/characters/jjory/core/character-jjory-happy.png'),
((SELECT id FROM character_types WHERE code = 'JJORY'), 'SLEEPY', 'assets/characters/jjory/core/character-jjory-sleepy.png'),
((SELECT id FROM character_types WHERE code = 'JJORY'), 'HUNGRY', 'assets/characters/jjory/status/character-jjory-hungry.png'),
((SELECT id FROM character_types WHERE code = 'JJORY'), 'LOW_ENERGY', 'assets/characters/jjory/status/character-jjory-low-energy.png'),
((SELECT id FROM character_types WHERE code = 'JJORY'), 'LONELY', 'assets/characters/jjory/status/character-jjory-lonely.png');
